...
func TestParseDetailPage_VidCodeReplacedWithMovieID(t *testing.T) {
	// When ScrapeURL is called with a ?v=<vid_code> URL (as generated by the
	// candidate picker), ExtractIDFromURL returns the vid code, not the movie ID.
	// extractMovieIDFromDetailHTML must resolve the real ID before parseDetailPage.
	settings := config.ScraperSettings{
		Enabled:  true,
		Language: "en",
		BaseURL:  "https://www.javlibrary.com",
	...
	html := `<html><head><title>HNDS-019 Some Great Title - JAVLibrary</title></head><body>
<div id="video_info"></div>
<img id="video_jacket_img" src="https://pics.dmm.co.jp/hnds019pl.jpg">
<div id="video_date"><td class="text">2026-01-01</td></div>
<div id="video_maker"><a href="/maker/test">Maker</a></div>
</body></html>`

	// Simulate what ScrapeURL does: start with the vid code, then replace with real ID.
	id := "javliob7y4"
	if realID := extractMovieIDFromDetailHTML(html); realID != "" {
		id = realID
	}

	result, err := s.parseDetailPage(html, id, "https://www.javlibrary.com/en/?v=javliob7y4", "en")
	if err != nil {
		t.Fatalf("parseDetailPage error: %v", err)
	}
	if result.ID != "HNDS-019" {
		t.Errorf("ID = %q, want HNDS-019 (not the vid code javliob7y4)", result.ID)
	}
	if result.Title != "Some Great Title" {
		t.Errorf("Title = %q, want Some Great Title", result.Title)
	}
}

func TestExtractMovieIDFromDetailHTML(t *testing.T) {
	tests := []struct {
		html string
		want string
	}{
		{`<title>HNDS-019 Some Title - JAVLibrary</title>`, "HNDS-019"},
		{`<title>IPX-535 Great Movie - JAVLibrary</title>`, "IPX-535"},
		{`<title>javliob7y4 - JAVLibrary</title>`, ""},  // vid code, no hyphen
		{`<title>Just a moment...</title>`, ""},          // CF challenge
		{`<div>no title</div>`, ""},
	}
	for _, tt := range tests {
		got := extractMovieIDFromDetailHTML(tt.html)
		if got != tt.want {
			t.Errorf("extractMovieIDFromDetailHTML(%q) = %q, want %q", tt.html, got, tt.want)
		}
	}
}
